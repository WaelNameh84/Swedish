import { Link } from "wouter";
import { AudioLines } from "lucide-react";
import { pronunciationTools } from "@/components/AppSidebar";

const DESCRIPTIONS: Record<string, string> = {
  "تسجيل صوتك": "سجّل نطقك لكلمة أو جملة واستمع إليه فوراً",
  "مقارنة النطق": "استمع للنطق الأصلي ثم لصوتك للمقارنة بينهما",
  "تقييم AI": "تقييم فوري لدقة نطقك بالذكاء الاصطناعي مع درجة ونصيحة",
  "تصحيح الأخطاء": "تحليل مفصّل للكلمات التي أخطأت في نطقها وسبب الخطأ",
  "مخارج الحروف": "تعرّف على مخارج أصوات سويدية صعبة وكيف تُنطق",
  "تمارين النطق": "تدرّب على نطق كلمات مختارة واحدة تلو الأخرى",
};

export default function PronunciationHubPage() {
  return (
    <div className="min-h-[100dvh] w-full max-w-2xl mx-auto pb-24">
      <header className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border px-4 py-4 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20">
          <AudioLines className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h1 className="text-lg font-bold text-foreground leading-tight">النطق</h1>
          <p className="text-xs text-muted-foreground mt-0.5">سجّل، قارن، وحسّن نطقك بالسويدية</p>
        </div>
      </header>

      <div className="p-4 grid grid-cols-2 gap-3">
        {pronunciationTools.map((tool) => {
          const Icon = tool.icon;
          return (
            <Link
              key={tool.href + tool.label}
              href={tool.href}
              className="flex flex-col items-start gap-3 p-4 rounded-2xl bg-card border border-card-border shadow-sm hover:border-primary/40 hover:shadow-md transition-all"
            >
              <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center">
                <Icon className="w-5 h-5 text-primary" />
              </div>
              <span className="text-sm font-semibold text-foreground leading-snug">{tool.label}</span>
              <span className="text-[11px] text-muted-foreground leading-snug -mt-1">
                {DESCRIPTIONS[tool.label]}
              </span>
            </Link>
          );
        })}
      </div>
    </div>
  );
}
